...
TextureCube gCubemap : environment
<
	string ResourceName = "";
	string UIWidget = "FilePicker";
...
int gCubemapSamplingMode <
   string UIName = "Cubemap Sampling Mode";
   string UIFieldNames ="Reflect:Refract:Normal:View";
   float UIMin = 0;
   float UIMax = 2;
   float UIStep = 1;
   int UIOrder = 21;
...
float4 ps(vertex2pixel In): SV_Target {
    ...
    float4 R;
    if (gCubemapSamplingMode == 0) {
        R = vec4(normalize(reflect(-V, N)));
    } else if (gCubemapSamplingMode == 1) {
        R = vec4(normalize(refract(-V, N, 1/gRefractIndex)));
    } else if (gCubemapSamplingMode == 2) {
        R = N;
    } else {
        R = -V;
    }
    ...
    // sample cubemap
    float4 reflectedColor = color4(gCubemap.SampleLevel(CubeMapSampler, R, gReflectionBlur));
